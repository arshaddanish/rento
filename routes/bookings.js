...
app.delete("/api/booking/:id", userAuth , async (req, res) => {
    try {
        const booking = await Booking.deleteOne({ _id: req.params.id, buyerId: req.user._id });
        if (!booking) {
            res.status(404).send();
        }
        res.send(booking);
...
app.get("/api/booking/requests/:usertype", userAuth ,async (req, res) => {
    try{
        ...
        if(req.params.usertype == "seller"){
            ...
            res.status(200).send({"bookings": bookings, "buyer_info": info, "item_info": itemInfo});
...
app.get("/api/booking/:usertype", userAuth ,async (req, res) => {
    try{
        ...
        res.status(200).send({"bookings": bookings, "seller_info": info , "item_info": itemInfo});
